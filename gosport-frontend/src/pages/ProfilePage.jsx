...
const ProfilePage = () => {
  ...
  const fetchTrainerStatus = async () => {
    try {
      const status = await profileApi.getTrainerStatus();
      setIsTrainer(status);
    } catch (err) {
      console.error("Failed to get trainer status", err);
    }
  };
  fetchTrainerStatus();
  }, []);
  ...
  <button
    onClick={async () => {
      try {
        await profileApi.toggleTrainerStatus();
        const updated = await profileApi.getTrainerStatus();
        setIsTrainer(updated);
      } catch (err) {
        console.error("Failed to toggle trainer status", err);
      }
    }}
    style={{ zIndex: 10 }}
  className={`relative mt-4 text-xs px-3 py-1 rounded-full shadow transition font-medium ${
    isTrainer
      ? "bg-green-600 text-white hover:bg-green-700"
      : "bg-gray-300 text-gray-700 hover:bg-gray-400"
  }`}
  >
    {isTrainer ? "Trainer (Click to unmark)" : "Mark yourself as trainer"}
  </button>
          </div>
        </div>
        ...
        <div className="pt-20 px-8 pb-8 relative">
          ...
          <div className="mb-6">
            ...
            <p className="text-xl text-gray-600 mt-2 break-words whitespace-pre-wrap">
